document.addEventListener('DOMContentLoaded', async (event) => {
    ...
    // HELPER: GET REFERRING PHYSICIANS LIST
    function getRefPhysicians() {
        const stored = localStorage.getItem('clinicRefPhysicians');
    ...
    if (currentReportType === 'nasal' || currentReportType === 'laryngeal') {
        ...
        // Referring Physician Logic
        const refSelect = document.getElementById('refPhysicianSelect');
        const refInput = document.getElementById('referringPhysician');
        
        // Populate Referring Physician Select
        if (refSelect) {
            const refPhysicians = getRefPhysicians();
            refSelect.innerHTML = '';
            ...
            refSelect.appendChild(otherOpt);
        }

        if (refSelect && refInput) {
            // Make the input field visible by default, it will now work alongside the dropdown
            ...
            refSelect.addEventListener('change', () => {
                if (refSelect.value === 'Other') {
                    ...
                    // This handles both a selected name and the empty "Select" option
                    refInput.value = refSelect.value;
                }
                refInput.dispatchEvent(new Event('input')); // Trigger auto-save
            });
        }
        ...
        function enhanceDropdowns() {
            ...
            const specificOptionsConfig = {
                ...
                // --- GENERAL ---
                'endoscopyType': ['Diagnostic Nasal Endoscopy', 'Flexible Nasopharyngolaryngoscopy', 'Rigid Laryngoscopy', 'Video Laryngoscopy', 'Nasopharyngoscopy']
            };

            let selectsToEnhance = [];

            // Assumes findings are in a fieldset with class 'findings-fieldset'
            const findingsFieldsets = document.querySelectorAll('.findings-fieldset');
            findingsFieldsets.forEach(fieldset => {
                const selects = fieldset.querySelectorAll('select');
                selects.forEach(s => selectsToEnhance.push(s));
            });

            // Also enhance Endoscopy Type if it's a select
            const typeSelect = document.getElementById('endoscopyType');
            if (typeSelect && typeSelect.tagName === 'SELECT') {
                selectsToEnhance.push(typeSelect);
            }

            if (selectsToEnhance.length === 0) return;

            selectsToEnhance.forEach(select => {
                const selectId = select.id;
                if (!selectId) return; // Skip selects without an ID
                ...
                // 2. Create the datalist element
                const datalist = document.createElement('datalist');
                datalist.id = datalistId;

                // 3. Use the config if available, otherwise use original options from the HTML
                const newOptions = specificOptionsConfig[selectId] || Array.from(select.options).map(opt => opt.value).filter(val => val);
                ...
                // 4. Replace the select with the new elements
                select.replaceWith(input, datalist);
            });
        }
        ...
        function loadDataIntoForm(data) {
            ...
            // Sync Referring Physician Select
            const refSelect = document.getElementById('refPhysicianSelect');
            const refInput = document.getElementById('referringPhysician');
            if (refSelect && refInput && data.referringPhysician) {
                // Ensure input is always visible when loading data
                refInput.style.display = 'block';
                const option = Array.from(refSelect.options).find(opt => opt.value === data.referringPhysician);
                if (option) {
                    refSelect.value = data.referringPhysician;
                } else {
                    refSelect.value = 'Other';
                }
            } else if (refInput) {
                refInput.style.display = 'block'; // Also ensure it's visible on a fresh form
            }
        }
        ...
        // ===================================================================================
        // PREVENT ACCIDENTAL NAVIGATION (Unsaved Changes Protection)
        // ===================================================================================
        // 1. Native Browser Check (Closing Tab/Window/Refresh)
        window.addEventListener('beforeunload', (e) => {
            const data = getFormData();
            // Check if there is significant data (Name, Images, or Impression)
            const hasData = (data.patientName && data.patientName.trim() !== '') || 
                            (data.images && data.images.length > 0) || 
                            (data.impression && data.impression.trim() !== '');
        ...
        // 2. Custom Modal for In-App Navigation (Back Button & Links)
        ...
        backLinks.forEach(link => {
            link.addEventListener('click', (e) => {
                ...
                if (hasData) {
                    ...
                    showCustomConfirm("You have an active report. Are you sure you want to leave?", (confirmed) => {
                        if (confirmed) {
                            // Proceed with navigation manually if confirmed
    ...
    if (actionsDiv && currentReportType !== 'settings') {
        ...
        if (backupBtn) {
            backupBtn.addEventListener('click', () => {
                const backupData = {
                    ...
                    clinicRefPhysicians: JSON.parse(localStorage.getItem('clinicRefPhysicians') || '[]'),
                    ...
                    try {
                        ...
                        showCustomConfirm("This will overwrite your current settings and history. Are you sure?", (confirmed) => {
                            if (confirmed) {
                                ...
                                if (importedData.clinicRefPhysicians) localStorage.setItem('clinicRefPhysicians', JSON.stringify(importedData.clinicRefPhysicians));
    ...
    if (currentReportType === 'settings') {
        ...
        if (previewBtn && previewContainer) {
            ...
            setupListManager('refPhysicianList', 'newRefPhysicianName', 'addRefPhysicianBtn', 'clinicRefPhysicians', 'Referring Physician');
                ...
                resetAllBtn.addEventListener('click', () => {
                    showCustomConfirm("Are you sure you want to reset ALL settings to default? This will delete your clinic details, physician list, and custom hospitals. This action cannot be undone.", (confirmed) => {
                        if (confirmed) {
                            ...
                            localStorage.removeItem('clinicRefPhysicians');
            ...
            try {
                ...
                if (leftInput && leftInput.files.length > 0) {
                    ...
                    if (leftInput.files[0].size > 500 * 1024) {
                        if(!confirm("Warning: The Left Logo file is large (" + (leftInput.files[0].size/1024).toFixed(0) + "KB). This might fill up your storage limit. Do you want to proceed?")) return;
                    }
                ...
                if (rightInput && rightInput.files.length > 0) {
                    ...
                    if (rightInput.files[0].size > 500 * 1024) {
                        if(!confirm("Warning: The Right Logo file is large (" + (rightInput.files[0].size/1024).toFixed(0) + "KB). This might fill up your storage limit. Do you want to proceed?")) return;
                    }
    ...
    function setupVideoToImage() {
        ...
        fixBtn.addEventListener('click', async () => {
            ...
            if (confirm("This will convert the video to a browser-friendly format (MP4/H.264). This fixes black screen issues but may take a moment. Proceed?")) {
    ...
    function setupAutoFields() {
        ...
        const generateImpression = () => {
            ...
            if (currentReportType === 'nasal') {
                // MODIFIED: Use .includes() to catch more specific values like "Deviated to the Left/Right"
                ...
                // MODIFIED: Use .includes() to catch more specific values like "Nodule (Bilateral)"
        ...
        const injectAutoButton = (inputId, generatorFn) => {
            ...
            if (input.parentNode.classList.contains('input-auto-wrapper')) return;
            ...
            btn.type = 'button';
            btn.innerHTML = '▼'; // Dropdown arrow icon
            btn.title = 'Options';
            ...
            // Fallback background
            if (!menu.style.backgroundColor) menu.style.backgroundColor = '#fff';

            // Helper to create menu items
            const createMenuItem = (text, icon, color, onClick) => {
                ...
                item.innerHTML = `<span style="font-size: 1.2em;">${icon}</span> <span>${text}</span>`;
                
                item.onmouseover = () => item.style.backgroundColor = 'rgba(0,0,0,0.05)';
            ...
            const itemGenerate = createMenuItem('Auto-Generate', '⚡', null, () => {
            ...
            // Option 2: Clear
            const itemClear = createMenuItem('Clear Field', '❌', 'var(--danger-color)', () => {
                input.value = '';
                input.dispatchEvent(new Event('input')); // Trigger auto-save
            });
            itemClear.style.borderBottom = 'none';

            menu.appendChild(itemGenerate);
            ...
            btn.onclick = (e) => {
                ...
                // Close other menus
                document.querySelectorAll('.input-auto-wrapper > div[style*="display: flex"]').forEach(m => m.style.display = 'none');
    ...
    function buildReportSections(data, findingsHtml) {
        const reportContent = `
            ...
            <p style="margin: 0 0 4px 0; font-size: 14px;"><strong>Name:</strong> <span>${escapeHtml(data.patientName) || 'N/A'}</span></p>
            <p style="margin: 0 0 4px 0;"><strong>Age:</strong> ${escapeHtml(data.patientAge) || 'N/A'}</p>
            <p style="margin: 0 0 4px 0;"><strong>Sex:</strong> ${escapeHtml(data.patientSex) || 'N/A'}</p>
            <p style="margin: 0 0 4px 0;"><strong>Date:</strong> ${formatDateForDisplay(data.procedureDate) || formatDateForDisplay(getTodayDate())}</p>
            ${data.referringPhysician ? `<p style="margin: 0 0 4px 0;"><strong>Referring Physician:</strong> ${escapeHtml(data.referringPhysician)}</p>` : ''}
            ...
            <p style="margin: 0 0 4px 0;"><strong>Type:</strong> ${escapeHtml(data.endoscopyType) || 'N/A'}</p>
        ...
        return { reportContent: reportContent + physicianBlock };
    }
    ...
    async function handleNativePrint() {
        ...
        const finalHtml = `
            ${headerHTML}
            <h2 style="text-align: center; font-size: 18px; margin: 0;">${reportTypeDisplay}</h2>
                    ...
                    if (typeof html2canvas === 'function' && (typeof window.jspdf !== 'undefined' || typeof window.jsPDF !== 'undefined')) {                    
                        ...
                        html2canvas(printDiv, { 
                            ...
                            if (showPreview) {
                                ...
                                // FIX: Adjust modal layout to ensure buttons are visible
                                // We assume the first child is the content container (e.g., .modal-content)
                                const modalContent = pdfPreviewModal.firstElementChild;
                                if (modalContent) {
                                    modalContent.style.display = 'flex';
                                    modalContent.style.flexDirection = 'column';
                                    modalContent.style.maxHeight = '90vh'; // Ensure it fits in viewport
                                    modalContent.style.overflow = 'hidden'; // Prevent outer scroll
                                }
                                // Make iframe take available space but leave room for buttons
                                pdfPreviewFrame.style.flex = '1';
                                pdfPreviewFrame.style.height = 'auto';
                                pdfPreviewFrame.style.minHeight = '0'; // Important for flex containers
